...
echo "=== Полная чистая установка TPM2-TSS 4.x + FAPI 3.x для Ubuntu 22.04 ==="

VENV_DIR="$HOME/tpmapp_venv"
BUILD_BASE="/tmp/tpm2-src"

echo "1) Удаление старых libtss2* (иначе ABI ломается)"
...
echo "2) Установка системных зависимостей (офиц. список)"
sudo apt update
sudo apt install -y \
  autoconf-archive \
  libcmocka0 \
...
echo "3) Скачивание исходников tpm2-tss"
rm -rf "$BUILD_BASE"
mkdir -p "$BUILD_BASE"
cd "$BUILD_BASE"
...
echo "5) Сборка и установка tpm2-tss"
make -j"$(nproc)"
sudo make install
sudo ldconfig

echo "6) Проверка FAPI (версия должна быть >= 3.x)"
if ! ldconfig -p | grep -q libtss2-fapi; then
    echo "FAPI не был установлен. Ошибка."
    exit 1
fi

echo "   libtss2-fapi найден:"
ldconfig -p | grep libtss2-fapi

echo "7) Скачивание и сборка tpm2-tools такой же версии"
cd "$BUILD_BASE"
...
echo "8) Создание python-venv и установка tpm2-pytss"
rm -rf "$VENV_DIR"
...
echo "Активируй окружение: source ~/tpmapp_venv/bin/activate"
